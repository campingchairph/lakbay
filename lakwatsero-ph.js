...
const S = {
  ...
  budgetAmount:20000, budgetType:'total', wizStep:0, isOwner:true, tripStart:null,
...
function renderMyBudget() {
  ...
  var tot = document.getElementById('mb-total');
  var cnt = document.getElementById('mb-count');
  if (tot) tot.textContent = '\u20b1'+total.toLocaleString();
  if (cnt) cnt.textContent = MY_EXPENSES.length + ' expense'+(MY_EXPENSES.length!==1?'s':'');
  ...
  list.innerHTML = MY_EXPENSES.map(function(e) {
    ...
    return '<div class="exp-card" style="margin-bottom:8px;">'
      ...
      +'<div><span style="font-size:15px;">'+(e.icon||'\U0001f4b8')+'</span> <strong style="font-size:13px;">'+e.label+'</strong></div>'
...
function deleteMbExpense(id) { MY_EXPENSES = MY_EXPENSES.filter(function(e){ return e.id!==id; }); renderMyBudget(); }
function editMbExpense(id) { openMyBudgetSheet(id); }
...
function updateWizFooter() {
  ...
  var step = S.wizStep;
  var blocked = false;
  if (step === 0 && !hasDest) blocked = true;
  if (step === 1) { var dv = document.getElementById('inp-startdate'); if (!dv || !dv.value) blocked = true; }
  if (step === 2) { var bv = parseFloat(document.getElementById('inp-budget') ? document.getElementById('inp-budget').value : 0)||0; if (!bv) blocked = true; }
  if (step === 0 && !hasDest) {
    ...
    nextBtn.style.opacity = blocked ? '.4' : '1';
...
function onStartDateInput() {
  ...
  if (hint && v) {
    ...
    hint.textContent = 'Trip starts ' + d.toLocaleDateString('en-PH',{weekday:'short',month:'short',day:'numeric'});
  }
}
...
function buildTripUI() {
  ...
  S.tripDays = Array.from({length:S.days}, function(_,i) {
    ...
    if (S.tripStart) {
      ...
      label = 'Day '+(i+1)+' \u00b7 '+d.toLocaleDateString('en-PH', opts);
    }
    return {id:'d'+(i+1), num:i+1, label:label, stops:[]};
  });
  renderDayTabs();
  buildBudgetDayTabs();
  renderExpenses();
...
function toggleRole(role) {
  ...
  if (idx === -1) { _pendingRoles.push(role); if (chip) chip.classList.add('active'); }
  else { _pendingRoles.splice(idx, 1); if (chip) chip.classList.remove('active'); }
}
function saveRoles() {
  ...
  if (m && _pendingRoles.length > 0) {
    ...
    m.role = primary; m.roles = _pendingRoles.slice();
...
function generateTripReport(mode, dayIndex, expenseId, personName) {
  ...
  if (mode === 'all') {
    ...
    reportSub   = S.days + ' days \xb7 ' + S.group + ' people \xb7 ' + _bd;
  } else if (mode === 'day') {
...
function openMyBudgetSheet(idOrNull) {
  ...
  _mbSelType = null; _mbItems = []; _mbReceipt = null;
  var prefill = idOrNull ? MY_EXPENSES.find(function(e){ return e.id===idOrNull; }) : null;
  var titleEl = document.getElementById('mb-sheet-title');
  if (titleEl) titleEl.textContent = idOrNull ? 'Edit Expense' : 'Log Personal Expense';
  var amtEl = document.getElementById('mb-amt'); if (amtEl) amtEl.value = prefill ? prefill.total : '';
  var noteEl = document.getElementById('mb-note-in'); if (noteEl) noteEl.value = prefill ? (prefill.note||'') : '';
  var rcpEl = document.getElementById('mb-receipt-lbl'); if (rcpEl) rcpEl.textContent = 'Tap to attach receipt photo';
  var formEl = document.getElementById('mb-form'); if (formEl) formEl.style.display = prefill ? 'block' : 'none';
  if (prefill) { _mbSelType = {icon:prefill.icon,label:prefill.label,cat:prefill.cat}; _mbItems = prefill.items||[]; }
  var chipsEl = document.getElementById('mb-type-chips');
  if (chipsEl) chipsEl.innerHTML = MB_TYPES.map(function(t){
...
function pickMbType(el, icon, label, cat) {
  ...
  var f = document.getElementById('mb-form'); if (f) f.style.display = 'block';
}

function addMbItem() { _mbItems.push({name:'',qty:1,price:0,subtotal:0}); renderMbItemsList(); }
...
function updateMbItem(i, field, val) {
  ...
  if (total>0) { var a = document.getElementById('mb-amt'); if(a) a.value = total; }
}
function removeMbItem(i) { _mbItems.splice(i,1); renderMbItemsList(); }
function handleMbReceipt(event) {
  var file = event.target.files[0]; if (!file) return;
  _mbReceipt = file.name;
  var el = document.getElementById('mb-receipt-lbl'); if (el) el.textContent = '\u2714 '+file.name;
}
function saveMbExpense() {
  ...
  var expData = {icon:_mbSelType.icon, label:_mbSelType.label, cat:_mbSelType.cat,
    total:amt, note:document.getElementById('mb-note-in').value.trim(),
    items:_mbItems.slice(), receipt:_mbReceipt};
  if (_mbEditingId) {
  ...
  } else { MY_EXPENSES.push(Object.assign({id:'mb'+Date.now()}, expData)); }
  closeSheet('sh-mybudget'); renderMyBudget();
}
...
function openMapForToday() {
  ...
  var query = encodeURIComponent(day.stops[0].name + (S.dest ? ' ' + S.dest : ''));
  var isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);
  window.open(isIOS ? 'maps://maps.apple.com/?q='+query : 'https://www.google.com/maps/search/?api=1&query='+query, '_blank');
}

/* --- TEST AS JOINER --- */
function testAsJoiner() {
  var name = prompt('Enter a test joiner name (blank for "Guest"):','');
